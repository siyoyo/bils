var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.sendMoneyComplete = function(response, data) {
		if (!(data["_extra"] && data["_extra"].convertedAccount) && !(data.recipient in NRS.contacts)) {
			$.growl("BILS已发送! <a href='#' data-account='" + NRS.getAccountFormatted(data, "recipient") + "' data-toggle='modal' data-target='#add_contact_modal' style='text-decoration:underline'>添加接收者为联系人?</a>", {
				"type": "success"
			});
		} else {
			$.growl("BILS已发送!", {
	...
	NRS.getAccountError = function(accountId, callback) {
		NRS.sendRequest("getAccount", {
			...
			if (response.publicKey) {
				callback({
					"type": "info",
					"message": "该接收者的帐户有公钥，且它的余额为" + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + "BILS.",
					"account": response
				});
			} else {
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": "该接收者的帐户有异常，请核实." + (!/^(BILS\-)/i.test(accountId) ? " 如果您是想输入别名, 请在它前面加上@." : ""),
							"account": null
						});
					...
					callback({
						"type": "warning",
						"message": "该接收者帐户没有公钥，这意味着该帐户没有向外发送过交易. 该帐户有余额" + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " BILS. 请再次检查确认一下您的接收者帐户地址是正确的.",
						"account": response
					});
				}
			}
	...
	NRS.checkRecipient = function(account, modal) {
		...
		if (/^(BILS\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {

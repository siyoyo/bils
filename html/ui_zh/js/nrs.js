var NRS = (function(NRS, $, undefined) {
	...
	NRS.state = {};
	NRS.blocks = [];
	NRS.genesis = "5547846998156852386";
			...
			if (response.errorCode) {
				...
				if (NRS.accountInfo.errorCode == 5) {
					if (NRS.downloadingBlockchain) {
						...
						$("#dashboard_message").addClass("alert-success").removeClass("alert-danger").html("欢迎光临您的新帐户. 您现在可以存一些BILS进来咯. 您的帐户ID是: <strong>" + String(preferredAccountFormat).escapeHTML() + "</strong>").show();
					}
				} else {
				...
				if (NRS.downloadingBlockchain) {
					...
					$("#dashboard_message").addClass("alert-danger").removeClass("alert-success").html("<b>请注意!</b>: 您的帐号没有公钥哦! 酱紫的话就不能像其它帐号一样受保护了. 您只要发送一笔款项，或者发送一条信息到其它帐户，或者购买一个别名就可以解决这问题了哦. (<a href='#' data-toggle='modal' data-target='#send_message_modal'>发送信息</a>, <a href='#' data-toggle='modal' data-target='#register_alias_modal'>购买别名</a>, <a href='#' data-toggle='modal' data-target='#send_money_modal'>发送BILS</a>, ...)").show();
				} else {

	...
	$("#account_details_modal").on("show.bs.modal", function(e) {
		...
		if (NRS.accountInfo.errorCode) {
			...
			if (NRS.accountInfo.errorCode == 5) {
				$("#account_balance_warning").html("您的帐户是全新的，建议您存入一些MYS，您的帐户ID是：<strong>" + NRS.account + "</strong>").show();
			} else {
			...
			$("#account_balance_balance").html(NRS.formatAmount(new BigInteger(NRS.accountInfo.balanceNQT)) + " MYS");
			$("#account_balance_unconfirmed_balance").html(NRS.formatAmount(new BigInteger(NRS.accountInfo.unconfirmedBalanceNQT)) + " MYS");
			$("#account_balance_effective_balance").html(NRS.formatAmount(NRS.accountInfo.effectiveBalanceMYS) + " MYS");
			$("#account_balance_guaranteed_balance").html(NRS.formatAmount(new BigInteger(NRS.accountInfo.guaranteedBalanceNQT)) + " MYS");
			...
			if (!NRS.accountInfo.publicKey) {
				...
				$("#account_balance_warning").html("您的帐号没有公钥哦! 酱紫的话就不能像其它帐号一样受保护了. 您只要发送一笔款项，或者发送一条信息到其它帐户，或者购买一个别名就可以解决这问题了哦. (<a href='#' data-toggle='modal' data-target='#send_message_modal'>发送消息</a>, <a href='#' data-toggle='modal' data-target='#register_alias_modal'>购买别名</a>, <a href='#' data-toggle='modal' data-target='#send_money_modal'>发送MYS</a>, ...)").show();
			}
		}
	});
